...
// ---------------- Lighter scanner: STARTED HERE (concurrent) so its paced waits cost no wall time ----------------
// No bulk positions endpoint (735k accounts), but trades expose account ids and each account payload
// includes the exchange-computed liquidation_price. Coverage builds cumulatively across runs.
// Rate limit ~100-req token bucket → paced single requests + 429 backoff + hard 5-min budget.
const lgPromise = (async () => {
  try {
    const LG = 'https://mainnet.zklighter.elliot.ai/api/v1';
    const lgEnd = Date.now() + 5 * 60_000; /* concurrent with the other book scans — bigger budget, same job length */
    ...
    try {
      ...
      for (const m of mkts) {
        if (Date.now() > lgEnd - 210_000) break; // reserve >=3.5 min for account scans
        try {
...
// ---------------- Lighter book: kicked off earlier (concurrent) — merge its result here ----------------
{
  const lgRes = await lgPromise;
  if (lgRes) {
    let lbj3 = { t: now };
